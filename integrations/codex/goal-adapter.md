...
Codex `/goal` is the outer objective runtime for ProofOps agents. The platform loop contract remains runtime-neutral; Codex only supplies a persistent goal driver for environments where the `goals` feature is enabled.
...
3. Render the adapter plan:
   ...
   ```bash
   npm run agents:goal-plan -- --agent proofops-governor --project-id my-project "Take this project through a release coverage matrix loop toward public-beta readiness"
   ```
