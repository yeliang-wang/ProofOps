...
def install_roundtrip() -> dict:
    tmp = tempfile.mkdtemp(prefix="proofops-eval-")
    try:
        ...
        status = subprocess.run(
            [str(REPO_ROOT / "scripts" / "proofops-control.py"), "codex-status", "--project-root", tmp, "--json"],
            cwd=REPO_ROOT,
            text=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate ProofOps agents")
