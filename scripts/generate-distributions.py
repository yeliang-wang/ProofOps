...
When running under Codex `/goal`, establish the loop goal window, release coverage matrix, and per-phase decision chain before starting or resuming: `finalGoal`, `phaseGoals`, `phaseSchemeOptions`, `phaseSchemeSources`, `recommendedPhaseSchemes`, `userSchemeSelection`, `repairMode`, `repairBoundaries`, `acceptanceCriteria`, `targetPlan`, `targetPlanConfirmation`, `reportCadence`, `finalDecision`, `coverageMatrix`, `evidenceMap`, `blockerPolicy`, `repairPolicy`, `releaseDecision`, and `decisionChain` must be explicit or product-native. Before any loop action starts, present the provided or inferred target plan to the user as a confirmation proposal, including the final target, phase targets, per-phase scheme options, recommendation, scheme source, tradeoff, repair mode, repair boundaries, per-phase acceptance criteria, release coverage matrix, evidence sources, blocker policy, repair policy, report cadence, and final decision vocabulary. Mark which parts came from the user, which came from product-native discovery, which came from current workspace discovery, which came from CI/CD or repository scripts, and which were inferred from ProofOps presets. Require explicit user confirmation and scheme selection before entering the loop. If the target plan is unconfirmed, stop as `BLOCKED: pending loop target plan confirmation`; if any phase target or scheme source is missing, stop as `BLOCKED: missing per-phase target or scheme source`; if repair would exceed confirmed boundaries, stop as `BLOCKED: repair exceeds confirmed boundaries`. Do not start, resume, or continue the loop until the user confirms or edits the plan and selects or confirms the recommended schemes and repair mode. Persist or report `loopState` after every iteration, keep confirmation gates authoritative, and stop instead of bypassing pending user approval, missing evidence, or a declared stop policy. Every phase report must print the decision chain that led to its conclusion: evidence used, rule applied, options considered, chosen decision, rejected alternatives when relevant, and next action. Persist summary state only in `loop-state.json`; large API responses, release decisions, risk registers, logs, screenshots, traces, and full evidence payloads must be externalized to iteration artifacts and referenced by path. JSONL loop logs must record event summaries, not full result payloads. Resume from the persisted attempt and product-native evidence store after restart, and enforce a state size guard before writing state so payload growth cannot terminate the loop. A loop that repeats the same blocker must switch from rerun mode into diagnosis, productized repair, and verification; if that cannot be done under the current permissions, stop as `BLOCKED` or `NO-GO`. Do not claim completion until the final goal, coverage matrix, decision chain, and acceptance criteria are evidence-proven. Do not weaken this agent's domain boundary merely because the outer runtime is continuous.
"""
