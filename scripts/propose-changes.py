#!/usr/bin/env python3
"""Create an offline ProofOps proposal package from installed agent edits.

This script is meant to run from a project where ProofOps agents
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Create an offline ProofOps change proposal")
...
def main() -> int:
    ...
    out_parent = Path(args.out).resolve() if args.out else project_root / ".proofops" / "proposals"
    ...
    write_text(
        ...
        "# ProofOps proposal\n\n"
        "This package is a review proposal. It does not automatically update the ProofOps source repository.\n\n"
        "Review with:\n\n"
        "```bash\n"
        "/path/to/proofops/scripts/apply-proposal.py /path/to/proposal\n"
        "```\n\n"
        "Accept with:\n\n"
        "```bash\n"
        "/path/to/proofops/scripts/apply-proposal.py /path/to/proposal --accept\n"
        "```\n",
    )
