...
if (!args.profile) {
  fail("Usage: node scripts/release-coverage-matrix-runner.mjs --profile <proofops.project.json> [--once] [--project-root <path>]");
}
...
const lifecycleRoot = path.join(projectRoot, "data", "proofops", lifecycleId);
...
function buildReleaseEvidenceBody({ step, coverageMatrix, latestSummary, attempt }) {
  ...
  return {
    ...
    source: "proofops release-coverage-matrix-runner",
    summary: compactSummary(unwrapData(latestSummary)),
    scenarioMatrix: [
      ...matrix,
